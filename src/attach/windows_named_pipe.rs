//! Attach API using a named pipe

use std::time::Duration;

use async_io::{
    os::windows::{NamedPipeListener, NamedPipeStream},
    Timer,
};
...
pub fn listen<A>() -> impl Stream<Item = Result<(NamedPipeStream, ()), Box<dyn std::error::Error>>>
where
    A: Attacher,
{
...
///
/// Returns the opened pipe on success.
pub async fn connect<A>(pid: u32) -> Result<NamedPipeStream, Box<dyn std::error::Error>>
where
    A: Attacher,
{
    let name = named_pipe_name(pid);
    ...
    while attempts < 100 {
        match NamedPipeStream::connect(&name).await {
            Ok(stream) => {
                return Ok(stream);
            }
            Err(err) => {
